...
namespace Mankibo
{
    public class World1BGScroll : MonoBehaviour
    {
        [Header("References")]
        ...
        private void Start()
        {
            ...
            {
                Debug.LogError("World1BG: No background layers assigned!");
                return;
            }
            ...
            StartCoroutine(ResolveSelectedCharacterName(selectedName =>
            {
                ...
                if (go == null)
                {
                    Debug.LogError($"World1BG: Character GameObject '{selectedName}' not found.");
                    return;
                }
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < layers.Length; i++)
            {
                float speed = (i < layerSpeeds.Length) ? layerSpeeds[i] : 0f;
