...
namespace Mankibo
{
    public class World1Controller : MonoBehaviour
    {
        ...
        private World1 character;               // Movement script on the selected character
        ...
        private void Start()
        {
            StartCoroutine(ResolveSelectedCharacterName(selectedName =>
            {
                ...
                if (go == null)
                {
                    Debug.LogError($"World1Controller: Character GameObject '{characterName}' not found in the scene.");
                    return;
                }

                character = go.GetComponent<World1>();
                if (character == null)
                {
                    Debug.LogError($"World1Controller: 'World1' component not found on '{characterName}'.");
                    return;
                }
