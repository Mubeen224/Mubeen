...
public class MathExitManagerW1 : MonoBehaviour
{
    // ===================== UI: Math Canvas =====================
    [Header("Math Canvas")]
    ...
    public List<LetterTracingW1> tracingScripts = new List<LetterTracingW1>();
    ...
    private LetterTracingW1 tracingScriptForReturn = null;
    ...
    private void Start()
    {
        if (playerScript == null) playerScript = FindObjectOfType<World1>();
    }
    ...
    public void OpenMathFromTracing(Action onCorrect, LetterTracingW1 scriptRef, GameObject canvasRef)
    {
        onSuccessCallback = onCorrect;
        wrongReturnDest = ReturnDest.Tracing;
    ...
    private void RestorePreviousUIAndState()
    {
        ...
        else // ReturnDest.AR
        {
            ...
            {
                var tracing = arCanvasForReturn ? arCanvasForReturn.GetComponentInChildren<LetterTracingW1>(true) : null;
    ...
    private void ReturnAfterWrong()
    {
        ...
        else // ReturnDest.AR
        {
            if (arCanvasForReturn != null)
            {
                ...
                {
                    var tracing = arCanvasForReturn.GetComponentInChildren<LetterTracingW1>(true);
                    if (tracing != null && tracing.arPopupAudio != null)
                        StartCoroutine(PlayAudioNextFrame(tracing.arPopupAudio));
                }
            }
    ...
    private void PrepareCanvasForInput(GameObject go)
    {
        if (!go) return;
        ...
        canvas.sortingOrder = 6000; // أعلى من أي popup عندك
    }
