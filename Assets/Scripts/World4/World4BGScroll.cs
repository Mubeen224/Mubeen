...
namespace Mankibo
{
    // Parallax background that follows the selected character in World 4
    public class World4BGScroll : MonoBehaviour
    {
        [Header("References")]
        ...
        private void Start()
        {
            ...
            {
                Debug.LogError("World4BG: No background layers assigned!");
                return;
            }
            ...
            // Bind to the selected character
            StartCoroutine(ResolveSelectedCharacterName(selectedName =>
            {
                var go = GameObject.Find(selectedName);
                if (go == null)
                {
                    Debug.LogError($"World4BG: Character GameObject '{selectedName}' not found.");
                    return;
                }
