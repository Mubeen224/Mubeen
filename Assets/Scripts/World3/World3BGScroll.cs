...
namespace Mankibo
{
    // Handles parallax background scrolling based on the selected character in World 3
    public class World3BGScroll : MonoBehaviour
    {
        [Header("References")]
        ...
        private void Start()
        {
            ...
            {
                Debug.LogError("World3BG: No background layers assigned!");
                return;
            }
            ...
            StartCoroutine(ResolveSelectedCharacterName(selectedName =>
            {
                ...
                if (go == null)
                {
                    Debug.LogError($"World3BG: Character GameObject '{selectedName}' not found.");
                    return;
                }
