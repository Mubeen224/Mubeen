...
public class WhisperSR3 : MonoBehaviour
{
    [Header("Whisper")]
    ...
    [Header("Next Step")]
    [Tooltip("كانفس AR الذي يُفتح بعد النجاح")]
    public GameObject moveToARCanvas;
    ...
    [TextArea] public string checkingMsgAr = "جارٍ التحقُّق من الإجابة ..."; // تمت إضافتها لتتوافق مع جزيرة 1
    ...
    [Header("Auto Stop on Voice (VAD)")]
    [Tooltip("إيقاف تلقائي عند التقاط طاقة صوت فوق العتبة")]
    ...
    [Tooltip("حد أمان: إيقاف إجباري بعد هذا الزمن (ث)")]
    ...
    private void Awake()
    {
        ...
        _player = FindFirstObjectByType<World3>(FindObjectsInactive.Include);
    }
    ...
    private void OnEnable()
    {
        ...
        // Ensure letter node exists (badge=false if missing)
        ...
        if (micButton)
        {
            ...
            micButton.interactable = false; // مثل الجزيرة الأولى: نغلقه مبدئياً
        }
    ...
    private async void OnRecordStop(AudioChunk recorded)
    {
        ...
        // مثل جزيرة 1: نعرض رسالة "جارٍ التحقق من الإجابة..."
        ...
        try
        {
            ...
            {
                Debug.LogError("[WhisperSR3] WhisperManager غير جاهز.");
                FailFlow();
                return;
            }
            ...
            Debug.Log($"[SR3] Raw='{text}'  Norm='{normalized}'  Target='{_targetLetter}'");
        ...
        {
            Debug.LogError($"[WhisperSR3] Exception: {e}");
            attemptErrorCount++;
            SaveAttemptError();
    ...
    private IEnumerator HandleSuccessOnce()
    {
        ...
        if (moveToARCanvas != null)
            moveToARCanvas.SetActive(true);
    ...
    private IEnumerator PlayOpening(float delay)
    {
        ...
        // السماح بالضغط فقط إذا المودل جاهز ولا يوجد تسجيل/تحليل
    ...
    private bool IsMatchLetterLenient(string recognizedNorm, string letter)
    {
        ...
        // ض
        if (L == "ض")
            accepted.AddRange(new[] { "ض", "ضا", "الضاد", "ضاد", "الضاء", "ضاء" });

        // ظ – هنا وسّعنا الاحتمالات وأضفنا كلمات قصيرة فيها ظ
        else if (L == "ظ")
            accepted.AddRange(new[]
            {
            "ظ", "ظا", "ظاء", "الظاء", "ظاد",
            "ظل",      // ممكن يكتبها المودل إذا قال الطفل "ظِل"
            "ظرف",     // لو استخدمتِ كلمة "ظرف"
            "ظبي"      // مثال آخر لو استخدمت كلمة فيها ظ
        });

        // غ
        else if (L == "غ")
            accepted.AddRange(new[] { "غ", "غا", "غين", "الغين", "غاء" });
        ...
        foreach (var tokRaw in tokens)
        {
            ...
            // 1) مطابقة مباشرة مع أي من accepted
            foreach (var a in accepted)
            {
                if (!string.IsNullOrEmpty(a) && tok == a)
                    return true;
                if (!string.IsNullOrEmpty(a) && tok.Contains(a))
                    return true;
            }

            // 2) حرف + مد (ضا / ظا / غا ... إلخ)
            if (tok.Length == 2 && tok[0] == targetChar &&
                (tok[1] == 'ا' || tok[1] == 'و' || tok[1] == 'ي'))
                return true;

            // 3) خاص للظاء: أي كلمة قصيرة تحتوي ظ تُعتبر صحيحة
            if (L == "ظ" && tok.Contains('ظ') && tok.Length <= 4)
                return true;

            // 4) لو النص بين علامات اقتباس
            var inner = tok.Trim('\'', '\"', '“', '”', '‘', '’');
            foreach (var a in accepted)
            {
                if (!string.IsNullOrEmpty(inner) && inner == a)
                    return true;
            }
        }
    ...
    private async Task<object> TranscribeFromChunk(AudioChunk chunk)
    {
        ...
        Debug.LogWarning("[WhisperSR3] لم يتم العثور على أي بيانات صوتية صالحة.");
        return string.Empty;
    }
    ...
    private string ExtractText(object result)
    {
        ...
        foreach (var name in new[] { "Text", "text", "Result", "result",
                                     "Transcript", "transcript", "FinalText", "finalText" })
        {
    ...
    private void SetMicVisual(bool recording)
    {
        ...
        if (legacy != null)
            legacy.text = recording ? "إيقاف" : "تسجيل";
    }
    ...
    private void UpdateLetterUI()
    {
        ...
        // مثل جزيرة 1: نعرض الحرف مع فتحة بسيطة ليثبت نطقه
    ...
    // ====================== Firebase: Ensure letter node (badge=false) ======================
    ...
    private void GiveCoinsSafely(int amount)
    {
        ...
        if (CoinsManager.instance != null)
        {
            try
            {
                ...
                Debug.Log($"[WhisperSR3] Coins added via CoinsManager (+{amount}).");
                return;
            }
            catch (Exception e)
            {
                Debug.LogWarning("[WhisperSR3] CoinsManager failed, will fallback to Firebase transaction: " + e);
            }
        }
    ...
    private IEnumerator AddCoinsDirectly(int amount)
    {
        ...
        {
            Debug.LogWarning("[WhisperSR3] Cannot add coins: parentId or selectedChildKey is empty.");
            yield break;
        }
        ...
        if (trxTask.Exception != null)
            Debug.LogWarning("[WhisperSR3] Firebase coins transaction failed: " + trxTask.Exception);
        else
            Debug.Log("[WhisperSR3] Coins added via Firebase transaction (+" + amount + ").");
    }

    // ===== Wait for Whisper model readiness =====
    private IEnumerator WaitForWhisperModelReady()
    {
        ...
        {
            _waitModelCR = null;
            yield break;   // ← هنا بدل return null
        }
        ...
        yield break;   // ← بدل return null
    }


    // ===== دالة خارجية لإيقاف التسجيل (للماث فاليديشن) =====
    public void StopRecordingExternal()
    {
        if (_isRecording)
        {
            SafeStopRecording();
            Debug.Log("[WhisperSR3] Recording stopped externally via StopRecordingExternal().");
        }
    }
}
